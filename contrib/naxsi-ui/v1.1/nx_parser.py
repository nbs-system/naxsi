...
class signature_extractor:
    def __init__(self):
        pass
    def count_per_exception(self, cursor, exception_id):
        cursor.execute("select count(DISTINCT srcpeer.peer_ip) as count from "
                       "peer "
        ...
        if data is None:
            return None
        return (data["count"])
    def gen_whitelists(self, cursor, d):
    ...
    def extract_whitelists(self, cursor):
        self.ret = self.extract_exceptions(cursor)
        print type(self.ret)
        for d in self.ret:
            count =  self.count_per_exception(cursor, d["id"])
            d["src_count"] = count
            self.wls = self.gen_whitelists(cursor, d)
    def extract_exceptions(self, cursor):
        cursor.execute("""select exception.exception_id as id, 
...
db = MySQLdb.connect("localhost", "root", "trivialpassword", "naxsi_sig")
cursor = db.cursor(MySQLdb.cursors.DictCursor)
bla = signature_extractor()
bla.extract_exceptions(cursor)
bla.extract_whitelists(cursor)
