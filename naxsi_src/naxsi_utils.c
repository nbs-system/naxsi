...
static int naxsi_unescape_uri(u_char **dst, u_char **src, size_t size,
                              ngx_uint_t type);
...
static ngx_int_t ngx_http_wlr_push_disabled(ngx_conf_t *cf,
                                            ngx_http_dummy_loc_conf_t *dlc,
                                            ngx_http_rule_t *curr) {
  ngx_http_rule_t **dr;

  if (!dlc->disabled_rules)
    dlc->disabled_rules =
        ngx_array_create(cf->pool, 4, sizeof(ngx_http_rule_t *));
  if (!dlc->disabled_rules)
    return (NGX_ERROR);
...
static ngx_int_t ngx_http_wlr_merge(ngx_conf_t *cf,
                                    ngx_http_whitelist_rule_t *father_wl,
                                    ngx_http_rule_t *curr) {
  uint i;
  ngx_int_t *tmp_ptr;

  NX_LOG_DEBUG(_debug_whitelist, NGX_LOG_EMERG, cf, 0,
               "[naxsi] merging similar wl(s)");
...
static ngx_int_t ngx_http_wlr_identify(ngx_conf_t *cf,
                                       ngx_http_dummy_loc_conf_t *dlc,
  ...
  for (i = 0; i < curr->br->custom_locations->nelts; i++) {
    ...
    if (custloc_array(curr->br->custom_locations->elts)[i].specific_url) {
      NX_LOG_DEBUG(
          _debug_whitelist_heavy, NGX_LOG_EMERG, cf, 0, "whitelist has URI %V",
    ...
    */
    if (custloc_array(curr->br->custom_locations->elts)[i].body_var) {
      NX_LOG_DEBUG(
          _debug_whitelist_heavy, NGX_LOG_EMERG, cf, 0,
      ...
      if (*name_idx != -1) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "whitelist can't target more than one BODY item.");
        return (NGX_ERROR);
      }
      *name_idx = i;
      *zone = BODY;
    ...
    if (custloc_array(curr->br->custom_locations->elts)[i].headers_var) {
      ...
      if (*name_idx != -1) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "whitelist can't target more than one HEADER item.");
        return (NGX_ERROR);
      }
      *name_idx = i;
      *zone = HEADERS;
    ...
    if (custloc_array(curr->br->custom_locations->elts)[i].args_var) {
      ...
      if (*name_idx != -1) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "whitelist can't target more than one ARGS item.");
        return (NGX_ERROR);
      }
...
ngx_http_whitelist_rule_t *ngx_http_wlr_find(ngx_conf_t *cf,
                                             ngx_http_dummy_loc_conf_t *dlc,
                                             ngx_http_rule_t *curr, int zone,
                                             int uri_idx, int name_idx,
                                             char **fullname) {
  ...
  if (uri_idx != -1 && name_idx != -1) {
    ...
    *fullname = ngx_pcalloc(
        ...
        custloc_array(curr->br->custom_locations->elts)[name_idx].target.len +
            custloc_array(curr->br->custom_locations->elts)[uri_idx]
                .target.len +
            3);
    ...
    strncat(
        *fullname,
        (const char *)custloc_array(curr->br->custom_locations->elts)[uri_idx]
            .target.data,
        custloc_array(curr->br->custom_locations->elts)[uri_idx].target.len);
    strncat(*fullname, (const char *)"#", 1);
    strncat(
        *fullname,
        (const char *)custloc_array(curr->br->custom_locations->elts)[name_idx]
            .target.data,
        custloc_array(curr->br->custom_locations->elts)[name_idx].target.len);
  }
  /* only uri */
  ...
  for (i = 0; i < dlc->tmp_wlr->nelts; i++)
    if (!strcmp(
            (const char *)*fullname,
            (const char *)((ngx_http_whitelist_rule_t *)dlc->tmp_wlr->elts)[i]
                .name->data) &&
...
static ngx_int_t
ngx_http_wlr_finalize_hashtables(ngx_conf_t *cf,
                                 ngx_http_dummy_loc_conf_t *dlc) {
  int get_sz = 0, headers_sz = 0, body_sz = 0, uri_sz = 0;
  ngx_array_t *get_ar = NULL, *headers_ar = NULL, *body_ar = NULL,
              *uri_ar = NULL;
...
ngx_int_t ngx_http_dummy_create_hashtables_n(ngx_http_dummy_loc_conf_t *dlc,
  ...
  if (!dlc->whitelist_rules || dlc->whitelist_rules->nelts == 0) {
    NX_LOG_DEBUG(_debug_whitelist_heavy, NGX_LOG_EMERG, cf, 0,
...
static const char *dummy_match_zones[] = {"HEADERS",  "URL",     "ARGS", "BODY",
...
void naxsi_log_offending(ngx_str_t *name, ngx_str_t *val,
  ...
  // encode uri
  tmp_uri.len =
      req->uri.len + (2 * ngx_escape_uri(NULL, req->uri.data, req->uri.len,
                                         NGX_ESCAPE_URI_COMPONENT));
  tmp_uri.data = ngx_pcalloc(req->pool, tmp_uri.len + 1);
  if (tmp_uri.data == NULL)
    return;
  ngx_escape_uri(tmp_uri.data, req->uri.data, req->uri.len,
                 NGX_ESCAPE_URI_COMPONENT);
  // encode val
  if (val->len <= 0)
    tmp_val = empty;
  ...
  if (name->len <= 0)
    ...
    if (tmp_name.data == NULL)
      return;
    ngx_escape_uri(tmp_name.data, name->data, name->len,
                   NGX_ESCAPE_URI_COMPONENT);
  }
  ...
  ngx_log_error(NGX_LOG_ERR, req->connection->log, 0,
                ...
                &(tmp_uri), rule->rule_id, dummy_match_zones[zone],
                target_name ? "|NAME" : "", &(tmp_name), &(tmp_val));
