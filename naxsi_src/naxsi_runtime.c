...
int ngx_http_dummy_is_whitelist_adapted(ngx_http_whitelist_rule_t *b,
										ngx_str_t *name,
										enum DUMMY_MATCH_ZONE zone,
										ngx_http_rule_t     *r,
										ngx_http_request_t  *req,
										...
										ngx_int_t target_name)
{
	...
	naxsi_whitelist_debug("Possible whitelist ... check...");

	/* if whitelist targets arg name, but the rules hit content*/
	if (b->target_name && !target_name) {
		naxsi_whitelist_debug("whitelist targets name, but rule matched content.");
		return 0;
	}

	/* if if the whitelist target contents, but the rule hit arg name*/
	if (!b->target_name && target_name) {
		naxsi_whitelist_debug("whitelist targets content, but rule matched name.");
		return 0;
	}


	if (type == NAME_ONLY) {
		naxsi_whitelist_debug("Name match in zone %s", zone == ARGS ? "ARGS" : zone == BODY ? "BODY" : zone == HEADERS ? "HEADERS" : "UNKNOWN!!!!!");
		...
		if (zone != b->zone || b->uri_only) {
			naxsi_whitelist_debug("bad whitelist, name match, but WL was only on URL.");
			return 0;
		}
	...
	if (type == URI_ONLY ||
		...
		if (b->uri_only && type != URI_ONLY) {
			naxsi_whitelist_debug("bad whitelist, type is URI_ONLY, but not whitelist");
			return 0;
		}

		if (zone != b->zone) {
			naxsi_whitelist_debug("bad whitelist, URL match, but not zone");
			return 0;
		}
	...
	naxsi_whitelist_debug("finished wl check, failed.");
...
ngx_http_whitelist_rule_t * nx_find_wl_in_hash(ngx_str_t *mstr,
		ngx_http_dummy_loc_conf_t *cf,
		enum DUMMY_MATCH_ZONE zone)
{
...
*/
int ngx_http_dummy_pcre_wrapper(ngx_regex_compile_t *rx, unsigned char *str, unsigned int len)
{
	int match;
	int captures[2];
...
#ifdef wlrx_debug
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "RXX - Trying to find rx for %v", name);
#endif
...
#ifdef wlrx_debug
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "Rule pushed to RXMZ, but has no custom_location.");
#endif
			continue;
		}
...
#ifdef wlrx_debug
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "%d/%d Not targeting same zone.");
#endif
			continue;
...
#ifdef wlrx_debug
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "only one target_name");
#endif
			continue;
		}
		...
		ngx_int_t target_name)
{
	...
	if (cf->disabled_rules) {
		...
		for (i = 0; i < cf->disabled_rules->nelts; i++) {
			...
			if (nx_check_ids(r->rule_id, dr[i]->wlid_array)) {
				...
				case ARGS:
					if (dr[i]->br->args) {
						naxsi_whitelist_debug("rule %d is disabled in ARGS", r->rule_id);
						return 1;
					}
				...
				case HEADERS:
					if (dr[i]->br->headers) {
						naxsi_whitelist_debug("rule %d is disabled in HEADERS", r->rule_id);
						return 1;
					}
				...
				case FILE_EXT:
					if (dr[i]->br->file_ext) {
						naxsi_whitelist_debug("rule %d is disabled in FILE_EXT", r->rule_id);
						return 1;
					}
				...
				case URL:
					if (dr[i]->br->url) {
						naxsi_whitelist_debug("rule %d is disabled in URL zone:%d", r->rule_id, zone);
						return 1;
					}
	...
	naxsi_whitelist_debug("hashing varname [%V]", name);
	...
	*/
	if (name->len > 0) {
		naxsi_whitelist_debug("hashing varname [%V] (rule:%d) - 'wl:X_VAR:%V'", name, r->rule_id, name);
		...
		naxsi_whitelist_debug("hashing varname [%V] (rule:%d) - 'wl:X_VAR:%V|NAME'", name, r->rule_id, name);
	...
	if (cf->wlr_url_hash && cf->wlr_url_hash->size > 0) {
		...
		naxsi_whitelist_debug("hashing uri [%V] (rule:%d) 'wl:$URI:%V|*'", &(tmp_hashname), r->rule_id, &(tmp_hashname));
	...
	naxsi_whitelist_debug("hashing uri#1 [%V] (rule:%d) ($URL:X|URI)", &(tmp_hashname), r->rule_id);
	b = nx_find_wl_in_hash(&(tmp_hashname), cf, zone);
	ngx_pfree(req->pool, tmp_hashname.data);
	tmp_hashname.data = NULL;
	...
	naxsi_whitelist_debug("hashing uri#3 [%V] (rule:%d) ($URL:X|ZONE|NAME)", &(tmp_hashname), r->rule_id);
	b = nx_find_wl_in_hash(&(tmp_hashname), cf, zone);
	ngx_pfree(req->pool, tmp_hashname.data);
	tmp_hashname.data = NULL;
	...
	naxsi_whitelist_debug("hashing MIX [%V] ($URL:x|$X_VAR:y) or ($URL:x|$X_VAR:y|NAME)", &tmp_hashname);
	b = nx_find_wl_in_hash(&(tmp_hashname), cf, zone);
	ngx_pfree(req->pool, tmp_hashname.data);
...
#ifdef wlrx_debug
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "Whitelisted by RX !");
#endif
...
ngx_str_t  *ngx_http_append_log(ngx_http_request_t *r, ngx_array_t *ostr, ngx_str_t *fragment, u_int *offset)
{
	u_int seed, sub;
...
ngx_int_t ngx_http_output_forbidden_page(ngx_http_request_ctx_t *ctx, ngx_http_request_t *r)
{
	...
	for (i = 0; i < ostr->nelts; i++) {
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "NAXSI_FMT: %s", ((ngx_str_t*)ostr->elts)[i].data);
	}
								 ...
								 ngx_int_t nb_match, ngx_int_t target_name)
{
	...
	if (ngx_http_dummy_is_rule_whitelisted_n(req, cf, r, name,
		...
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "rule is whitelisted.");
#endif
		return 0;
	}
	...
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "Current extensive log value: %d", ctx->extensive_log);
#endif
	...
	if (r->sscores) {
		...
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "Rule applies %d custom scores", r->sscores->nelts);
#endif
						  ...
						  enum DUMMY_MATCH_ZONE zone)
{
	...
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "XX-check url-like [%s]", str);
#endif
	...
	while (str < (orig + full_len) && *str) {
...
#ifdef spliturl_ruleset_debug
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "XX-url has no '&' and '=' or has both [%s]", str);
#endif
...
#ifdef spliturl_ruleset_debug
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "XX-url has no '=' but has '&' [%s]", str);
#endif
...
#ifdef spliturl_ruleset_debug
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "XX-Classic format url [%s]", str);
#endif
		...
		if (rules) {
			...
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "XX-no arg rules ?");
		}
		...
		if (main_rules) {
			...
			ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "XX-no main rules ?");
		}
							   ...
							   enum DUMMY_MATCH_ZONE zone)
{
	...
	r = rules->elts;
#ifdef basestr_ruleset_debug
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "XX-checking rules ...");
#endif
		...
		*/
		...
		if (filen_start) {
...
#ifdef post_heavy_debug
			else { // here we got val name + val content!
				ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "[POST] No local body rules");
			}
			...
			if (main_cf->body_rules) {
				...
				ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "[POST] No main body rules");
			}
...
#ifdef dummy_body_parse_debug
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "XX-No content type ..");
#endif
...
#ifdef dummy_body_parse_debug
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "XX-VALID BODY");
#endif
...
#ifdef post_heavy_debug
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "XX-application/x-www..");
#endif
		...
		ngx_http_request_t *r)
{
	...
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "XX-custom check rules");
#endif
...
void ngx_http_dummy_payload_handler(ngx_http_request_t *r)
{
	...
	ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "XX-dummy PAYLOAD HANDLER !");
#endif

	if (ctx->wait_for_body) {
#ifdef payload_handler_debug
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "XX-dummy : AFTER NGX_AGAIN");
#endif
		ctx->wait_for_body = 0;
