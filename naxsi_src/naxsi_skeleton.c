...
*/
#define ngx_http_dummy_line_conf_error(cf, value)                            \
    do                                                                       \
    {                                                                        \
                           ...
                           void *conf)
{
    ...
    // move to next word
    while (value[1].data[i] && !(value[1].data[i] >= '0' && value[1].data[i] <= '9') && (value[1].data[i] != '-'))
        i++;
    NX_LOG_DEBUG(_debug_readconf, NGX_LOG_EMERG, cf, 0,
...
static ngx_int_t ngx_http_dummy_access_handler(ngx_http_request_t *r)
{
    ...
    */
        ...
        if ((r->method == NGX_HTTP_PATCH || r->method == NGX_HTTP_POST || r->method == NGX_HTTP_PUT) && !ctx->ready)
        {
            NX_DEBUG(_debug_mechanics, NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
