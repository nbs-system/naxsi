...
static char             *ngx_http_dummy_merge_loc_conf(ngx_conf_t *cf,
		void *parent,
		void *child);
static void                    *ngx_http_dummy_create_main_conf(ngx_conf_t *cf);
...
static void * ngx_http_dummy_create_loc_conf(ngx_conf_t *cf)
{
	return ngx_pcalloc(cf->pool, sizeof(ngx_http_dummy_loc_conf_t));
}
...
*/
static char * ngx_http_dummy_read_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	...
	if (!ngx_strcmp(value[0].data, TOP_BASIC_RULE_T) ||
		...
		} else { // else push in appropriate ruleset
			if (rule.br->headers) {
#ifdef readconf_debug
				ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
...
static char * ngx_http_naxsi_cr_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
...
static char * ngx_http_naxsi_ud_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	ngx_http_dummy_loc_conf_t     *alcf = conf, **bar;
	ngx_http_dummy_main_conf_t    *main_cf;
	ngx_str_t                     *value;
...
static char * ngx_http_naxsi_flags_loc_conf(ngx_conf_t *cf, ngx_command_t *cmd,
		void *conf)
{
	ngx_http_dummy_loc_conf_t     *alcf = conf, **bar;
	ngx_http_dummy_main_conf_t    *main_cf;
	...
	if (!ngx_strcmp(value[0].data, TOP_ENABLED_FLAG_T) ||
		...
		alcf->enabled = 1;
		return NGX_CONF_OK;
	} else if (!ngx_strcmp(value[0].data, TOP_DISABLED_FLAG_T) ||
			   !ngx_strcmp(value[0].data, TOP_DISABLED_FLAG_N)) {
		/* it's a flagrule, just a hack to enable/disable mod */
		alcf->force_disabled = 1;
		return NGX_CONF_OK;
	} else if (!ngx_strcmp(value[0].data, TOP_LEARNING_FLAG_T) ||
		...
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "LibInjectionSql enabled");
#endif
...
#ifdef loc_conf_debug
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "LibInjectionXss enabled");
#endif
		return NGX_CONF_OK;
	} else {
		return NGX_CONF_ERROR;
...
static ngx_int_t ngx_http_dummy_access_handler(ngx_http_request_t *r)
{
	...
	if (ctx && ctx->wait_for_body) {
		...
		ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "naxsi:NGX_AGAIN");
#endif
		return NGX_DONE;
	}
