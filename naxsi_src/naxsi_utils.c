...
** '%uff' -> 'uff'
*/
int naxsi_unescape_uri(u_char **dst, u_char **src, size_t size, ngx_uint_t type)
{
...
ngx_int_t ngx_http_wlr_push_disabled(ngx_conf_t *cf, ngx_http_dummy_loc_conf_t *dlc,
									 ngx_http_rule_t *curr)
{
	ngx_http_rule_t       **dr;

#ifdef whitelist_debug
	ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "[naxsi] rule %d disabled", curr->wl_id[0]);
#endif
...
ngx_int_t ngx_http_wlr_merge(ngx_conf_t *cf, ngx_http_whitelist_rule_t *father_wl,
							 ngx_http_rule_t *curr)
{
	uint i;
	ngx_int_t             *tmp_ptr;
...
ngx_int_t ngx_http_wlr_identify(ngx_conf_t *cf, ngx_http_dummy_loc_conf_t *dlc,
								...
								int *uri_idx, int *name_idx)
{
	...
	// if we're facing a WL in the style $URL:/bla|ARGS (or any other zone), push it to
...
ngx_http_whitelist_rule_t* ngx_http_wlr_find(ngx_conf_t *cf, ngx_http_dummy_loc_conf_t *dlc,
		ngx_http_rule_t *curr, int zone, int uri_idx, int name_idx, char **fullname)
{
	uint i;
...
#ifdef whitelist_heavy_debug
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "whitelist has uri + name");
#endif
...
#ifdef whitelist_heavy_debug
			ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "whitelist targets |NAME");
#endif
...
#ifdef whitelist_heavy_debug
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "whitelist has uri");
#endif
...
ngx_int_t ngx_http_wlr_finalize_hashtables(ngx_conf_t *cf, ngx_http_dummy_loc_conf_t  *dlc)
{
...
#ifdef whitelist_heavy_debug
	ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "finalizing hashtables");
#endif
...
ngx_int_t ngx_http_dummy_create_hashtables_n(ngx_http_dummy_loc_conf_t *dlc, ngx_conf_t *cf)
{
...
#ifdef whitelist_heavy_debug
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "No whitelist registred, but it's your call.");
#endif
		return NGX_OK;
	}
...
#ifdef whitelist_heavy_debug
			ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "WL %d is a disable rule.", i);
#endif
						 ...
						 enum DUMMY_MATCH_ZONE zone, ngx_int_t target_name)
{
	...
	char tmp_zone[30] = {0};	/* 30 is more than enough */
