                                         ...
                                         ngx_int_t target_name)
{
    ...
    NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log, 0, "is rule [%d] whitelisted in zone %s for item %V", r->rule_id,
             zone == ARGS ? "ARGS" : zone == HEADERS ? "HEADERS" : zone == BODY ? "BODY" : zone == URL ? "URL" : zone == FILE_EXT ? "FILE_EXT" : zone == RAW_BODY ? "RAW_BODY" : "UNKNOWN",
             name);
    if (target_name)
                               ...
                               enum DUMMY_MATCH_ZONE zone)
{
    ...
    NX_DEBUG(_debug_basestr_ruleset, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
             ...
             zone == BODY ? "BODY" : zone == HEADERS ? "HEADERS" : zone == URL ? "URL" : zone == ARGS ? "ARGS" : zone == FILE_EXT ? "FILE_EXT" : zone == RAW_BODY ? "RAW_BODY" : "UNKNOWN");
