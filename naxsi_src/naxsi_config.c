...
static char *dummy_id(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);
static char *dummy_score(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);
static char *dummy_msg(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);
static char *dummy_rx(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);
static char *dummy_zone(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);
static char *dummy_str(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);
static char *dummy_negative(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);
static char *dummy_whitelist(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);

/*  Structures related to the configuration parser */
static const struct {
	char  *prefix;
	char  *(*pars)(ngx_conf_t *, ngx_str_t *, ngx_http_rule_t *);
} rule_parser[] = {
	{ ID_T,		dummy_id	},
	{ SCORE_T,	dummy_score	},
	{ MSG_T,	dummy_msg	},
...
static char* dummy_zone(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule)
{
	...
	while (*tmp_ptr) {
		...
		if (!strncmp(tmp_ptr, "BODY", strlen("BODY"))) {
			...
			tmp_ptr += strlen("ARGS");
			continue;
		} else if (!strncmp(tmp_ptr, "NAME", strlen("NAME"))) { // match against variable name
			...
			continue;
		} else if (tmp_ptr[0] == '$') { /* custom match zones */
			...
			if (!strncmp(tmp_ptr, "$ARGS_VAR:", strlen("$ARGS_VAR:"))) {
				custom_rule->args_var = 1;
				rule->br->args_var = 1;
				tmp_ptr += strlen("$ARGS_VAR:");
			} else if (!strncmp(tmp_ptr, "$BODY_VAR:", strlen("$BODY_VAR:"))) {
				custom_rule->body_var = 1;
				rule->br->body_var = 1;
				tmp_ptr += strlen("$BODY_VAR:");
			} else if (!strncmp(tmp_ptr, "$HEADERS_VAR:", strlen("$HEADERS_VAR:"))) {
				custom_rule->headers_var = 1;
				rule->br->headers_var = 1;
				tmp_ptr += strlen("$HEADERS_VAR:");
			} else if (!strncmp(tmp_ptr, "$URL:", strlen("$URL:"))) {
				...
				** messy as hell
				*/
				if (!strncmp(tmp_ptr, "$ARGS_VAR_X:", strlen("$ARGS_VAR_X:"))) {
					custom_rule->args_var = 1;
					rule->br->args_var = 1;
					rule->br->rx_mz = 1;
					tmp_ptr += strlen("$ARGS_VAR_X:");
				} else if (!strncmp(tmp_ptr, "$BODY_VAR_X:", strlen("$BODY_VAR_X:"))) {
					custom_rule->body_var = 1;
					rule->br->body_var = 1;
					rule->br->rx_mz = 1;
					tmp_ptr += strlen("$BODY_VAR_X:");
				} else if (!strncmp(tmp_ptr, "$HEADERS_VAR_X:", strlen("$HEADERS_VAR_X:"))) {
					custom_rule->headers_var = 1;
					rule->br->headers_var = 1;
					rule->br->rx_mz = 1;
					tmp_ptr += strlen("$HEADERS_VAR_X:");
				} else if (!strncmp(tmp_ptr, "$URL_X:", strlen("$URL_X:"))) {
					custom_rule->specific_url = 1;
					rule->br->rx_mz = 1;
					tmp_ptr += strlen("$URL_X:");
				} else {
					return NGX_CONF_ERROR;
				}
			}
...
static char* dummy_rx(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule)
{
...
#ifdef rx_debug
		ngx_conf_log_error(NGX_LOG_EMERG, r, 0, "XX-FAILED RX:%V", tmp);
#endif
		return NGX_CONF_ERROR;
	}
	...
	rule->br->rx = rgc;
#ifdef rx_debug
	ngx_conf_log_error(NGX_LOG_EMERG, r, 0, "XX- RX:[%V]", &(rule->br->rx->pattern));
#endif
	return NGX_CONF_OK;
}
	...
	*/
...
#ifdef dummy_cfg_parse_one_rule_debug
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "XX-crit in rule %V", &(value[1]));
#endif
		return NGX_CONF_ERROR;
	}
	...
	for (i = 1; i < nb_elem && value[i].len > 0; i++) {
		...
		for (z = 0; rule_parser[z].pars; z++) {
			if (!ngx_strncmp(value[i].data, rule_parser[z].prefix, strlen(rule_parser[z].prefix))) {
				ret = rule_parser[z].pars(cf, &(value[i]), current_rule);

				if (ret != NGX_CONF_OK) {
#ifdef dummy_cfg_parse_one_rule_debug
					ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "XX-FAILED PARSING '%s'", value[i].data);
#endif
					return ret;
				}
