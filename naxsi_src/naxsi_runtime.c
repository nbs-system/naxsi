...
ngx_http_rule_t nx_int__weird_request = {
    ...
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 1,
...
ngx_http_rule_t nx_int__big_request = {
    /*type*/ 0,         /*whitelist flag*/ 0,
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 2,
    /*log_msg*/ NULL,   /*score*/ 0,
    /*sscores*/ NULL,
    /*sc_block*/ 0,     /*sc_allow*/ 0,
    /*block*/ 1,        /*allow*/ 0,          /*drop*/ 0,    /*log*/ 0,
...
ngx_http_rule_t nx_int__uncommon_hex_encoding = {
    /*type*/ 0,         /*whitelist flag*/ 0,
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 10,
    /*log_msg*/ NULL,   /*score*/ 0,
    /*sscores*/ NULL,
    /*sc_block*/ 1,     /*sc_allow*/ 0,
    /*block*/ 1,        /*allow*/ 0,          /*drop*/ 0,
    /*log*/ 0,
    /*br ptrs*/ NULL};

ngx_http_rule_t nx_int__uncommon_content_type = {
    /*type*/ 0,         /*whitelist flag*/ 0,
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 11,
    /*log_msg*/ NULL,   /*score*/ 0,
    /*sscores*/ NULL,
    /*sc_block*/ 1,     /*sc_allow*/ 0,
    /*block*/ 1,        /*allow*/ 0,          /*drop*/ 0,
...
ngx_http_rule_t nx_int__uncommon_url = {
    /*type*/ 0,         /*whitelist flag*/ 0,
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 12,
    /*log_msg*/ NULL,   /*score*/ 0,
    /*sscores*/ NULL,
    /*sc_block*/ 1,     /*sc_allow*/ 0,
    /*block*/ 1,        /*allow*/ 0,          /*drop*/ 0,
...
ngx_http_rule_t nx_int__uncommon_post_format = {
    /*type*/ 0,         /*whitelist flag*/ 0,
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 13,
    /*log_msg*/ NULL,   /*score*/ 0,
    /*sscores*/ NULL,
    /*sc_block*/ 1,     /*sc_allow*/ 0,
    /*block*/ 1,        /*allow*/ 0,          /*drop*/ 0,
...
ngx_http_rule_t nx_int__uncommon_post_boundary = {
    /*type*/ 0,         /*whitelist flag*/ 0,
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 14,
    /*log_msg*/ NULL,   /*score*/ 0,
    /*sscores*/ NULL,
    /*sc_block*/ 1,     /*sc_allow*/ 0,
    /*block*/ 1,        /*allow*/ 0,          /*drop*/ 0,
...
ngx_http_rule_t nx_int__empty_post_body = {
    /*type*/ 0,         /*whitelist flag*/ 0,
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 16,
    /*log_msg*/ NULL,   /*score*/ 0,
    /*sscores*/ NULL,
    /*sc_block*/ 1,     /*sc_allow*/ 0,
    /*block*/ 1,        /*allow*/ 0,          /*drop*/ 0,
...
ngx_http_rule_t nx_int__no_rules = {
    /*type*/ 0,         /*whitelist flag*/ 0,
    /*wl_id ptr*/ NULL, /*pass_id ptr*/ NULL, /*rule_id*/ 19,
    /*log_msg*/ NULL,   /*score*/ 0,
    /*sscores*/ NULL,
    /*sc_block*/ 0,     /*sc_allow*/ 0,
    /*block*/ 0,        /*allow*/ 0,          /*drop*/ 1,
    /*log*/ 0,
    /*br ptrs*/ NULL};
...
#define dummy_error_fatal(ctx, r, ...)                                         \
  do {                                                                         \
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,                   \
                  "XX-func:%s file:%s line:%d", __func__, __FILE__, __LINE__); \
    if (r && r->uri.data)                                                      \
      ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "XX-uri:%s",    \
                    r->uri.data);                                              \
  } while (0)
...
int ngx_http_basestr_ruleset_n(ngx_pool_t *pool, ngx_str_t *name,
                               ngx_str_t *value, ngx_array_t *rules,
...
void naxsi_log_offending(ngx_str_t *name, ngx_str_t *val,
                         ngx_http_request_t *req, ngx_http_rule_t *rule,
                         enum DUMMY_MATCH_ZONE zone, ngx_int_t target_name);
void ngx_http_dummy_rawbody_parse(ngx_http_request_ctx_t *ctx,
                                  ngx_http_request_t *r, u_char *src,
                                  u_int len);
...

{
  ...
  *nb_match = 0;
  if (rl->br->match_type == RX && rl->br->rx) {
    ...
    (nginx_version >= 1002002 && nginx_version != 1003000)
        (tmp_idx < len &&
         (match = pcre_exec(rl->br->rx->regex->code, 0, (const char *)str->data,
                            str->len, tmp_idx, 0, captures, 30)) >= 0)
#elif defined nginx_version && (nginx_version > 1001011)
        (tmp_idx < len &&
         (match = pcre_exec(rl->br->rx->regex->pcre, 0, (const char *)str->data,
                            str->len, tmp_idx, 0, captures, 30)) >= 0)
#elif defined nginx_version && (nginx_version <= 1001011)
        (tmp_idx < len &&
         (match = pcre_exec(rl->br->rx->regex, 0, (const char *)str->data,
                            str->len, tmp_idx, 0, captures, 30)) >= 0)
...
#error "nginx_version not defined."
    ...
    libinjection_sqli_init(&state, (const char *)str->data, str->len,
                           FLAG_NONE);
...
int ngx_http_dummy_is_whitelist_adapted(
    ngx_http_whitelist_rule_t *b, ngx_str_t *name, enum DUMMY_MATCH_ZONE zone,
    ngx_http_rule_t *r, ngx_http_request_t *req, enum MATCH_TYPE type,
    ngx_int_t target_name) {
  NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
  ...
  if (b->target_name && !target_name) {
    NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "whitelist targets name, but rule matched content.");
    return (0);
  }
  /* if if the whitelist target contents, but the rule hit arg name*/
  if (!b->target_name && target_name) {
    NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "whitelist targets content, but rule matched name.");
    return (0);
  }

  if (type == NAME_ONLY) {
    NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "Name match in zone %s",
             zone == ARGS ? "ARGS" : zone == BODY
                                         ? "BODY"
    ...
    if (zone != b->zone || b->uri_only) {
      NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
...
ngx_http_whitelist_rule_t *nx_find_wl_in_hash(ngx_http_request_t *req,
                                              ngx_str_t *mstr,
                                              ngx_http_dummy_loc_conf_t *cf,
                                              enum DUMMY_MATCH_ZONE zone) {
  ngx_int_t k;
  ...
  if ((zone == BODY || zone == FILE_EXT) && cf->wlr_body_hash &&
 ...
 *well.
 */
int ngx_http_dummy_pcre_wrapper(ngx_regex_compile_t *rx, unsigned char *str,
                                unsigned int len) {
  int match;
  int captures[30];
    ...
    (nginx_version >= 1002002 && nginx_version != 1003000)
  match =
      pcre_exec(rx->regex->code, 0, (const char *)str, len, 0, 0, captures, 1);
#elif defined nginx_version && (nginx_version > 1001011)
  match =
      pcre_exec(rx->regex->pcre, 0, (const char *)str, len, 0, 0, captures, 1);
#elif defined nginx_version && (nginx_version <= 1001011)
...
int ngx_http_dummy_is_rule_whitelisted_rx(ngx_http_request_t *req,
                                          ngx_http_dummy_loc_conf_t *cf,
  ...
  for (i = 0; i < cf->rxmz_wlr->nelts; i++) {
    ...
    if (!p->br || !p->br->custom_locations ||
        p->br->custom_locations->nelts < 1) {
      NX_DEBUG(wl_debug_rx, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
    ...
    for (x = 0, violation = 0;
         x < p->br->custom_locations->nelts && violation == 0; x++) {
      /* does custom location targets a body var ? */
      if (custloc_array(p->br->custom_locations->elts)[x].body_var) {
        rx_match = ngx_http_dummy_pcre_wrapper(
            custloc_array(p->br->custom_locations->elts)[x].target_rx,
            name->data, name->len);
        if (rx_match < 0) {
          violation = 1;
          NX_DEBUG(wl_debug_rx, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
                   "[BODY] FAIL:%d (rx:%V, str:%V)", rx_match,
                   &(custloc_array(p->br->custom_locations->elts)[x]
                         .target_rx->pattern),
                   name);
        ...
        NX_DEBUG(wl_debug_rx, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
                 "[BODY] Match:%d (rx:%V, str:%V)", rx_match,
                 &(custloc_array(p->br->custom_locations->elts)[x]
                       .target_rx->pattern),
                 name);
      }
      ...
      if (custloc_array(p->br->custom_locations->elts)[x].args_var) {
        ...
        if (rx_match < 0) {
          violation = 1;
          NX_DEBUG(wl_debug_rx, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
                   "[ARGS] FAIL:%d (rx:%V, str:%V)", rx_match,
                   &(custloc_array(p->br->custom_locations->elts)[x]
                         .target_rx->pattern),
                   name);
        ...
        NX_DEBUG(wl_debug_rx, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
                 "[ARGS] Match:%d (rx:%V, str:%V)", rx_match,
                 &(custloc_array(p->br->custom_locations->elts)[x]
                       .target_rx->pattern),
                 name);
      }
      ...
      if (custloc_array(p->br->custom_locations->elts)[x].specific_url) {
        ...
        if (rx_match < 0) {
          NX_DEBUG(wl_debug_rx, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
                   "[URI] FAIL:%d (rx:%V, str:%V)", rx_match,
                   &(custloc_array(p->br->custom_locations->elts)[x]
                         .target_rx->pattern),
                   &(req->uri));
        ...
        NX_DEBUG(wl_debug_rx, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
                 "[URI] Match:%d (rx:%V, str:%V)", rx_match,
                 &(custloc_array(p->br->custom_locations->elts)[x]
                       .target_rx->pattern),
                 &(req->uri));
      }
...
int ngx_http_dummy_is_rule_whitelisted_n(ngx_http_request_t *req,
                                         ngx_http_dummy_loc_conf_t *cf,
                                         ngx_http_rule_t *r, ngx_str_t *name,
                                         enum DUMMY_MATCH_ZONE zone,
                                         ngx_int_t target_name) {
  ...
  if (target_name)
    NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "extra: exception happened in |NAME");
  ...
  if (cf->disabled_rules) {
    ...
    for (i = 0; i < cf->disabled_rules->nelts; i++) {
      ...
      if (nx_check_ids(r->rule_id, dr[i]->wlid_array)) {
        ...
        NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
                 req->connection->log, 0, "rule %d is disabled somewhere",
                 r->rule_id);
        /* if it doesn't specify zone, skip zone-check */
        if (!dr[i]->br) {
          NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
                   req->connection->log, 0, "no zone, skip zone-check");
          continue;
        }

        /* If rule target nothing, it's whitelisted everywhere */
        if (!(dr[i]->br->args || dr[i]->br->headers || dr[i]->br->body ||
              dr[i]->br->url)) {
          NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
                   req->connection->log, 0, "rule %d is fully disabled",
                   r->rule_id);
          return (1);
        }
        ...
        case ARGS:
          if (dr[i]->br->args) {
            NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
                     req->connection->log, 0, "rule %d is disabled in ARGS",
                     r->rule_id);
            return (1);
          }
          break;
        case HEADERS:
          if (dr[i]->br->headers) {
            NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
                     req->connection->log, 0,
                     "rule %d is disabled in HEADERS with IP", r->rule_id);
            return (1);
          }
          break;
        case BODY:
          if (dr[i]->br->body) {
            NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
                     req->connection->log, 0, "rule %d is disabled in BODY",
                     r->rule_id);
            return (1);
          }
          break;
        ...
        case FILE_EXT:
          if (dr[i]->br->file_ext) {
            NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
                     req->connection->log, 0, "rule %d is disabled in FILE_EXT",
                     r->rule_id);
            return (1);
          }
          break;
        case URL:
          if (dr[i]->br->url) {
            NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP,
                     req->connection->log, 0,
                     "rule %d is disabled in URL zone:%d", r->rule_id, zone);
            return (1);
          }
          break;
        default:
  ...
  if (name->len > 0) {
    NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "hashing varname [%V] (rule:%d) - 'wl:X_VAR:%V'", name,
             r->rule_id, name);
    /* try to find in hashtables */
    b = nx_find_wl_in_hash(req, name, cf, zone);
    if (b && ngx_http_dummy_is_whitelist_adapted(b, name, zone, r, req,
                                                 NAME_ONLY, target_name))
      return (1);
    ...
    NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "hashing varname [%V] (rule:%d) - 'wl:X_VAR:%V|NAME'", name,
             r->rule_id, name);
  ...
  if (cf->wlr_url_hash && cf->wlr_url_hash->size > 0) {
    ...
    NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "hashing uri [%V] (rule:%d) 'wl:$URI:%V|*'", &(tmp_hashname),
             r->rule_id, &(tmp_hashname));
  ...
  NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
           "hashing uri#1 [%V] (rule:%d) ($URL:X|URI)", &(tmp_hashname),
           r->rule_id);
  b = nx_find_wl_in_hash(req, &(tmp_hashname), cf, zone);
  ngx_pfree(req->pool, tmp_hashname.data);
  tmp_hashname.data = NULL;
  if (b && ngx_http_dummy_is_whitelist_adapted(b, name, zone, r, req, URI_ONLY,
                                               target_name))
    return (1);
  ...
  NX_DEBUG(_debug_whitelist_compat, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
           "hashing uri#3 [%V] (rule:%d) ($URL:X|ZONE|NAME)", &(tmp_hashname),
           r->rule_id);
  b = nx_find_wl_in_hash(req, &(tmp_hashname), cf, zone);
  ngx_pfree(req->pool, tmp_hashname.data);
  tmp_hashname.data = NULL;
  ...
  if (b && ngx_http_dummy_is_whitelist_adapted(b, name, zone, r, req, MIXED,
                                               target_name))
    return (1);
   ...
   ** Look it up in regexed whitelists for matchzones
   */
  if (ngx_http_dummy_is_rule_whitelisted_rx(req, cf, r, name, zone,
                                            target_name) == 1) {
    NX_DEBUG(wl_debug_rx, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
             "Whitelisted by RX !");
...
ngx_str_t *ngx_http_append_log(ngx_http_request_t *r, ngx_array_t *ostr,
  ...
  while ((seed = random() % 1000) == prev_seed)
    ;
  sub = snprintf((char *)(fragment->data + *offset), MAX_SEED_LEN,
                 "&seed_start=%d", seed);
...
ngx_int_t ngx_http_nx_log(ngx_http_request_ctx_t *ctx, ngx_http_request_t *r,
                          ngx_array_t *ostr, ngx_str_t **ret_uri) {
  u_int sz_left, sub, offset = 0, i;
  ngx_str_t *fragment, *tmp_uri;
  ngx_http_special_score_t *sc;
  const char *fmt_base = "ip=%.*s&server=%.*s&uri=%.*s&learning=%d&vers=%.*s&"
                         "total_processed=%zu&total_blocked=%zu&block=%d";
  ...
  *ret_uri = tmp_uri;
   ...
   */
  sub = snprintf((char *)fragment->data, sz_left, fmt_base,
                 ...
                 strlen(NAXSI_VERSION), NAXSI_VERSION, cf->request_processed,
                 cf->request_blocked, ctx->block ? 1 : (ctx->drop ? 1 : 0));
  ...
  for (i = 0; ctx->special_scores && i < ctx->special_scores->nelts; i++) {
    ...
    if (sc[i].sc_score != 0) {
      sub = snprintf(0, 0, fmt_score, i, sc[i].sc_tag->len, sc[i].sc_tag->data,
                     i, sc[i].sc_score);
      if (sub >= sz_left) {
        /*
      ...
      sub = snprintf((char *)(fragment->data + offset), sz_left, fmt_score, i,
                     sc[i].sc_tag->len, sc[i].sc_tag->data, i, sc[i].sc_score);
      if (sub >= sz_left)
        sub = sz_left - 1;
      offset += sub;
      sz_left -= sub;
  ...
  if (ctx->matched) {
    ...
    do {
      ...
      tmp_val.len = mr[i].name->len +
                    (2 * ngx_escape_uri(NULL, mr[i].name->data, mr[i].name->len,
                                        NGX_ESCAPE_URI_COMPONENT));

      tmp_val.data = ngx_pcalloc(r->pool, tmp_val.len + 1);
      ngx_escape_uri(tmp_val.data, mr[i].name->data, mr[i].name->len,
                     NGX_ESCAPE_URI_COMPONENT);

      sub = snprintf(0, 0, fmt_rm, i, tmp_zone, i, mr[i].rule->rule_id, i,
      ...
      sub = snprintf((char *)fragment->data + offset, sz_left, fmt_rm, i,
                     tmp_zone, i, mr[i].rule->rule_id, i, tmp_val.len,
                     tmp_val.data);
      if (sub >= sz_left)
        sub = sz_left - 1;
      offset += sub;
...
#define NAXSI_HEADER_NAXSI_SIG "x-naxsi_sig"
  ...
  if (r->headers_in.headers.last) {
    ...
    memcpy(h->lowcase_key, NAXSI_HEADER_ORIG_URL,
           strlen(NAXSI_HEADER_ORIG_URL));
    ...
    memcpy(h->lowcase_key, NAXSI_HEADER_ORIG_ARGS,
           strlen(NAXSI_HEADER_ORIG_ARGS));
    ...
    memcpy(h->lowcase_key, NAXSI_HEADER_NAXSI_SIG,
           strlen(NAXSI_HEADER_NAXSI_SIG));
 ...
 */
int ngx_http_apply_rulematch_v_n(ngx_http_rule_t *r,
                                 ngx_http_request_ctx_t *ctx,
  ...
  if (!cf || !ctx)
    return (0);
  if (ngx_http_dummy_is_rule_whitelisted_n(req, cf, r, name, zone,
                                           target_name) == 1) {

    NX_DEBUG(_debug_whitelist_light, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "rule is whitelisted.");
  ...
  if (!ctx->matched)
    ctx->matched =
        ngx_array_create(req->pool, 2, sizeof(ngx_http_matched_rule_t));
    ...
    for (z = 0; z < r->sscores->nelts; z++) {
      ...
      for (i = 0; i < ctx->special_scores->nelts; i++) {
        if (rsc[z].sc_tag && sc[i].sc_tag &&
          ...
          NX_DEBUG(_debug_whitelist, NGX_LOG_DEBUG_HTTP, req->connection->log,
                   0, "Special Score (%V) actual=%d,next=%d", rsc[z].sc_tag,
                   sc[i].sc_score,
      ...
      if (!found) {
        NX_DEBUG(_debug_whitelist, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
                 "Special Score (%V)  next=%d", rsc[z].sc_tag,
 ...
 *audited :)
 */
int ngx_http_spliturl_ruleset(ngx_pool_t *pool, ngx_str_t *nx_str,
                              ngx_array_t *rules, ngx_array_t *main_rules,
                              ngx_http_request_t *req,
                              ngx_http_request_ctx_t *ctx,
                              enum DUMMY_MATCH_ZONE zone) {
  ngx_str_t name, val;
  ...
  if (naxsi_escape_nullbytes(nx_str) > 0) {
    ...
    dummy.data = NULL;
    dummy.len = 0;
    ngx_http_apply_rulematch_v_n(&nx_int__uncommon_hex_encoding, ctx, req,
                                 &dummy, &dummy, zone, 1, 0);
  }
  ...
  while (str < (orig + full_len) && *str) {
    ...
    if ((!eq && !ev) /*?foobar */ ||
      ...
      NX_DEBUG(_debug_spliturl_ruleset, NGX_LOG_DEBUG_HTTP,
               req->connection->log, 0,
    ...
    /* ?&&val | ?var&& | ?val& | ?&val | ?val&var */
      ...
      NX_DEBUG(_debug_spliturl_ruleset, NGX_LOG_DEBUG_HTTP,
               req->connection->log, 0, "XX-url has no '=' but has '&' [%s]",
               str);

      ngx_http_apply_rulematch_v_n(&nx_int__uncommon_url, ctx, req, NULL, NULL,
                                   zone, 1, 0);
      ...
      NX_DEBUG(_debug_spliturl_ruleset, NGX_LOG_DEBUG_HTTP,
               req->connection->log, 0, "XX-Classic format url [%s]", str);
      ...
      eq = strnchr(str, '=', len);
      if (!eq) {
        if (ngx_http_apply_rulematch_v_n(&nx_int__uncommon_url, ctx, req, NULL,
                                         NULL, zone, 1, 0)) {
          dummy_error_fatal(ctx, req, "malformed url, possible attack [%s]",
                            str);
        }
        return (1);
      }
    ...
    NX_DEBUG(_debug_spliturl_ruleset, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "XX-extract  [%V]=[%V]", &(name), &(val));

    if (rules)
      ngx_http_basestr_ruleset_n(pool, &name, &val, rules, req, ctx, zone);
    else
      NX_DEBUG(_debug_spliturl_ruleset, NGX_LOG_DEBUG_HTTP,
               req->connection->log, 0, "XX-no arg rules ?");

    if (main_rules)
      ngx_http_basestr_ruleset_n(pool, &name, &val, main_rules, req, ctx, zone);
    else
      NX_DEBUG(_debug_spliturl_ruleset, NGX_LOG_DEBUG_HTTP,
               req->connection->log, 0, "XX-no main rules ?");
...
void ngx_http_libinjection(ngx_pool_t *pool, ngx_str_t *name, ngx_str_t *value,
                           ngx_http_request_ctx_t *ctx, ngx_http_request_t *req,
                           enum DUMMY_MATCH_ZONE zone) {
  /*
  ...
  if (ctx->libinjection_sql) {
     ...
     */
    libinjection_sqli_init(&state, (const char *)name->data, name->len,
                           FLAG_NONE);
    issqli = libinjection_is_sqli(&state);
    if (issqli == 1) {
      ngx_http_apply_rulematch_v_n(nx_int__libinject_sql, ctx, req, name, value,
                                   zone, 1, 1);
    }

    /* hardcoded call to libinjection on CONTENT, apply internal rule if
     * matched. */
    libinjection_sqli_init(&state, (const char *)value->data, value->len,
                           FLAG_NONE);
    issqli = libinjection_is_sqli(&state);
    if (issqli == 1) {
      ngx_http_apply_rulematch_v_n(nx_int__libinject_sql, ctx, req, name, value,
  ...
  if (ctx->libinjection_xss) {
    ...
    if (issqli == 1) {
      ngx_http_apply_rulematch_v_n(nx_int__libinject_xss, ctx, req, name, value,
                                   zone, 1, 1);
    }
...
int ngx_http_basestr_ruleset_n(ngx_pool_t *pool, ngx_str_t *name,
  ...
  for (i = 0;
       i < rules->nelts && ((!ctx->block || ctx->learning) && !ctx->drop);
       i++) {
    /*properly reset counter*/
    uri_constraint_ok = 1;
    rule_matched = 0;
    NX_DEBUG(_debug_basestr_ruleset, NGX_LOG_DEBUG_HTTP, req->connection->log,
             0, "XX-RULE %d : START", r[i].rule_id);
    ...
    if (name && r[i].br->custom_location) {
      ...
      for (z = 0; z < r[i].br->custom_locations->nelts; z++) {
        ...
        if (location[z].specific_url) {
          ...
          if (r[i].br->rx_mz) {
            ...
            if (ngx_http_dummy_pcre_wrapper(
                    location[z].target_rx, req->uri.data, req->uri.len) == -1) {
              uri_constraint_ok = 0;
            }
          }
      ...
      if (uri_constraint_ok == 0) {
        NX_DEBUG(_debug_basestr_ruleset, NGX_LOG_DEBUG_HTTP,
                 req->connection->log, 0, "XX URI CONSTRAINT MISMATCH, SKIP");
        continue;
      }
      ...
      for (z = 0; z < r[i].br->custom_locations->nelts; z++) {
         ...
         * ARGS_VAR_X / ..) */
        if (r[i].br->rx_mz &&
            ngx_http_dummy_pcre_wrapper(location[z].target_rx, name->data,
                                        name->len) == -1)
          continue;
         ...
         * / ..) */
        if ((!r[i].br->rx_mz) &&
            (name->len != location[z].target.len ||
        ...
        if (!r[i].br->negative) {
          ...
          ngx_log_error(NGX_LOG_ERR, req->connection->log, 0, "check name: %s",
                        name);
          ...
          if (ret == 1) {
            NX_DEBUG(_debug_basestr_ruleset, NGX_LOG_DEBUG_HTTP,
                     req->connection->log, 0, "XX-apply rulematch[in name] "
                                              "[%V]=[%V] [rule=%d] (match %d "
                                              "times)",
                     name, value, r[i].rule_id, nb_match);
            rule_matched = 1;
            ngx_http_apply_rulematch_v_n(&(r[i]), ctx, req, name, name, zone,
                                         nb_match, 1);
          }
        }
        if (rule_matched == 1) {
          NX_DEBUG(_debug_basestr_ruleset, NGX_LOG_DEBUG_HTTP,
                   req->connection->log, 0, "XX-[SPECIFIC] Rule %d matched in "
                                            "custom_location, go to next rule",
                   r[i].rule_id);
    ...
    if ((zone == HEADERS && r[i].br->headers) ||
        (zone == URL && r[i].br->url) || (zone == ARGS && r[i].br->args) ||
      ...
      if (!r[i].br->target_name) {
        NX_DEBUG(_debug_basestr_ruleset, NGX_LOG_DEBUG_HTTP,
                 req->connection->log, 0, "XX-test rulematch (value) "
                                          "[zone-wide]!1 [%V]=[%V] [rule =%d] "
                                          "(%d times)",
                 name, value, r[i].rule_id, nb_match);
        ...
        if (ret == 1) {
          ...
          // ngx_http_apply_rulematch_v_n(&(r[i]), ctx, req, name, value, zone,
          // nb_match, 0);
          ngx_http_apply_rulematch_v_n(&(r[i]), ctx, req, name, value, zone, 0,
                                       0);
        }
      }
      ...
      if (!r[i].br->negative) {
        NX_DEBUG(
            _debug_basestr_ruleset, NGX_LOG_DEBUG_HTTP, req->connection->log, 0,
        ...
        if (ret == 1) {
          NX_DEBUG(_debug_basestr_ruleset, NGX_LOG_DEBUG_HTTP,
                   req->connection->log, 0,
                   "XX-apply rulematch!1 [%V]=[%V] [rule=%d] (%d times)", name,
...
int nx_content_type_parse(ngx_http_request_t *r, unsigned char **boundary,
                          unsigned int *boundary_len) {
  unsigned char *h;
  unsigned char *end;

  h = r->headers_in.content_type->value.data + strlen("multipart/form-data;");
  end = r->headers_in.content_type->value.data +
...
void ngx_http_dummy_multipart_parse(ngx_http_request_ctx_t *ctx,
  ...
  /*extract boundary*/
  if (nx_content_type_parse(r, (unsigned char **)&boundary, &boundary_len) !=
      NGX_OK) {
    if (boundary && boundary_len > 1)
      NX_DEBUG(_debug_post_heavy, NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
               "XX-POST boundary : (%s) : %d", boundary, boundary_len);
    ngx_http_apply_rulematch_v_n(&nx_int__uncommon_post_boundary, ctx, r, NULL,
                                 NULL, BODY, 1, 0);
    return;
  }
  NX_DEBUG(_debug_post_heavy, NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
  ...
  while (idx < len) {
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                  "Remaining Len:%d (boundary len:%d)", len - idx,
                  boundary_len);
    ...
    if (idx + boundary_len + 6 == len || idx + boundary_len + 4 == len) {
      if (ngx_strncmp(src + idx, "--", 2) ||
        ...
        ngx_http_apply_rulematch_v_n(&nx_int__uncommon_post_boundary, ctx, r,
                                     NULL, NULL, BODY, 1, 0);
        return;
      } else
        break;
    ...
    /* --boundary\r\n : New var */
    if ((len - idx < 4 + boundary_len) || src[idx] != '-' ||
        src[idx + 1] != '-' ||
        ...
        src[idx + boundary_len + 2] != '\r' ||
        src[idx + boundary_len + 3] != '\n') {
      /* bad boundary */
      ngx_http_apply_rulematch_v_n(&nx_int__uncommon_post_boundary, ctx, r,
                                   NULL, NULL, BODY, 1, 0);
      return;
    ...
    if (ngx_strncasecmp(src + idx, (u_char *)"content-disposition: form-data;",
                        31)) {
      ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                    "Unknown content-type: [%s]", src + idx);
      if (ngx_http_apply_rulematch_v_n(&nx_int__uncommon_post_format, ctx, r,
                                       NULL, NULL, BODY, 1, 0)) {
    ...
    if (nx_content_disposition_parse(src + idx, line_end, &varn_start,
                                     &varn_end, &filen_start, &filen_end,
                                     r) != NGX_OK) {
      ngx_http_apply_rulematch_v_n(&nx_int__uncommon_post_format, ctx, r, NULL,
                                   NULL, BODY, 1, 0);
      return;
    }
    ...
    if (filen_start && filen_end) {
      ...
      if (!line_end) {
        if (ngx_http_apply_rulematch_v_n(&nx_int__uncommon_post_format, ctx, r,
                                         NULL, NULL, BODY, 1, 0)) {
          dummy_error_fatal(
              ctx, r, "POST data : malformed filename (no content-type ?)");
        }
        return;
      }
    }
    ...
    if (src[idx] != '\r' || src[idx + 1] != '\n') {
      if (ngx_http_apply_rulematch_v_n(&nx_int__uncommon_post_format, ctx, r,
        ...
        dummy_error_fatal(ctx, r,
                          "POST data : malformed content-disposition line");
      }
      return;
    }
    idx += 2;
    ...
    if (filen_start) {
      ...
      if (nullbytes > 0) {
        ngx_http_apply_rulematch_v_n(&nx_int__uncommon_hex_encoding, ctx, r,
                                     &final_var, &final_data, BODY, 1, 1);
      }
      nullbytes = naxsi_unescape(&final_data);
      if (nullbytes > 0) {
        ngx_http_apply_rulematch_v_n(&nx_int__uncommon_hex_encoding, ctx, r,
                                     &final_var, &final_data, BODY, 1, 0);
      }

      NX_DEBUG(_debug_post_heavy, NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
               "[POST] checking filename [%V] = [%V]", &final_var, &final_data);
...
void ngx_http_dummy_body_parse(ngx_http_request_ctx_t *ctx,
  ...
  if (!r->request_body->bufs) {
    ngx_http_apply_rulematch_v_n(&nx_int__empty_post_body, ctx, r, NULL, NULL,
                                 BODY, 1, 0);
    return;
  }
  if (!r->headers_in.content_type) {
    ...
    ngx_http_apply_rulematch_v_n(&nx_int__uncommon_content_type, ctx, r, NULL,
                                 NULL, BODY, 1, 0);
    return;
  }
  ...
  if (r->request_body->temp_file) {
    ...
    ngx_http_apply_rulematch_v_n(&nx_int__big_request, ctx, r, NULL, NULL, BODY,
                                 1, 0);
    return;
  }
  ...
  /* request body in chain */
    ...
    for (bb = r->request_body->bufs; bb; bb = bb->next)
      full_body =
          ngx_cpymem(full_body, bb->buf->pos, bb->buf->last - bb->buf->pos);
    full_body = src;
    NX_DEBUG(_debug_body_parse, NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
             "[POST] REQUEST BODY IN CHAIN [%s] (len=%d)", full_body,
             full_body_len);
  }
  ...
  if (!ngx_strncasecmp(r->headers_in.content_type->value.data,
    ...
    if (ngx_http_spliturl_ruleset(r->pool, &tmp, cf->body_rules,
                                  main_cf->body_rules, r, ctx, BODY)) {
      ngx_http_apply_rulematch_v_n(&nx_int__uncommon_url, ctx, r, NULL, NULL,
                                   BODY, 1, 0);
      return;
    }
  }
 ...
 */
void ngx_http_dummy_uri_parse(ngx_http_dummy_main_conf_t *main_cf,
                              ngx_http_dummy_loc_conf_t *cf,
                              ngx_http_request_ctx_t *ctx,
                              ngx_http_request_t *r) {
  ngx_str_t tmp, name;
  ...
  if (!main_cf->generic_rules && !cf->generic_rules) {
    tmp.data = NULL;
    tmp.len = 0;
    ngx_http_apply_rulematch_v_n(&nx_int__no_rules, ctx, r, &tmp, &tmp, URL, 1,
                                 0);
    return;
  }
  tmp.len = r->uri.len;
  ...
  if (naxsi_escape_nullbytes(&tmp) > 0) {
    ...
    ngx_http_apply_rulematch_v_n(&nx_int__uncommon_hex_encoding, ctx, r,
                                 &tmp_name, &tmp_val, URL, 1, 0);
  }
  name.data = NULL;
  name.len = 0;
  if (cf->generic_rules)
    ngx_http_basestr_ruleset_n(r->pool, &name, &tmp, cf->generic_rules, r, ctx,
                               URL);
  if (main_cf->generic_rules)
    ngx_http_basestr_ruleset_n(r->pool, &name, &tmp, main_cf->generic_rules, r,
                               ctx, URL);
  ngx_pfree(r->pool, tmp.data);
}

void ngx_http_dummy_args_parse(ngx_http_dummy_main_conf_t *main_cf,
                               ngx_http_dummy_loc_conf_t *cf,
                               ngx_http_request_ctx_t *ctx,
                               ngx_http_request_t *r) {
  ngx_str_t tmp;
  ...
  if (ngx_http_spliturl_ruleset(r->pool, &tmp, cf->get_rules,
                                main_cf->get_rules, r, ctx, ARGS)) {
    dummy_error_fatal(ctx, r,
...
void ngx_http_dummy_headers_parse(ngx_http_dummy_main_conf_t *main_cf,
                                  ngx_http_dummy_loc_conf_t *cf,
                                  ngx_http_request_ctx_t *ctx,
                                  ngx_http_request_t *r) {
  ...
  for (i = 0; ((!ctx->block || ctx->learning) && !ctx->block); i++) {
    ...
    if (naxsi_escape_nullbytes(&lowcase_header) > 0) {
      ngx_http_apply_rulematch_v_n(&nx_int__uncommon_hex_encoding, ctx, r,
                                   &h[i].key, &h[i].value, HEADERS, 1, 1);
    }
    if (naxsi_escape_nullbytes(&h[i].value) > 0) {
      ngx_http_apply_rulematch_v_n(&nx_int__uncommon_hex_encoding, ctx, r,
                                   &h[i].key, &h[i].value, HEADERS, 1, 0);
    }
    if (cf->header_rules)
...
void ngx_http_dummy_update_current_ctx_status(ngx_http_request_ctx_t *ctx,
  ...
  if (r->headers_in.x_forwarded_for.nelts >= 1) {
    ...
    if (n >= 1) {
      ...
      ngx_str_t tmp_hashname2;
      tmp_hashname2.len = strlen((char *)h[0]->value.data);
      tmp_hashname2.data = ngx_pcalloc(r->pool, tmp_hashname2.len + 1);
      memcpy(tmp_hashname2.data, h[0]->value.data, 15);
      b = nx_find_pass_in_hash(r, &tmp_hashname2, cf, zone);
    }
  }
  /*cr, sc, cf, ctx*/
