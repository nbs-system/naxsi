...
void *dummy_pass(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule);
/*
...
void *dummy_score(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule) {
  ...
  NX_LOG_DEBUG(_debug_score, NGX_LOG_EMERG, r, 0, "XX-(debug) dummy score (%V)",
               tmp);
  /*allocate scores array*/
  if (!rule->sscores) {
    rule->sscores =
        ngx_array_create(r->pool, 1, sizeof(ngx_http_special_score_t));
  }

  while (*tmp_ptr) {
    if (tmp_ptr[0] == '$') {
      NX_LOG_DEBUG(_debug_score, NGX_LOG_EMERG, r, 0,
                   "XX-(debug) special scoring rule (%s)", tmp_ptr);
      ...
      NX_LOG_DEBUG(_debug_score, NGX_LOG_EMERG, r, 0,
                   "XX-(debug) special scoring (%V) => (%d)", sc->sc_tag,
                   sc->sc_score);

      /* move to end of score. */
      while (/*don't overflow*/ ((unsigned int)((unsigned char *)tmp_ptr -
                                                tmp->data)) < tmp->len &&
  ...
  if (rule->sscores) {
    for (z = 0; z < rule->sscores->nelts; z++) {
      ngx_conf_log_error(NGX_LOG_EMERG, r, 0,
                         "XX-score n°%d special scoring (%V) => (%d)", z,
                         scr[z].sc_tag, scr[z].sc_score);
    }
  } else
    ngx_conf_log_error(NGX_LOG_EMERG, r, 0,
...
#define MZ_SPECIFIC_URL_T "$URL:"
      ...
      if (!rule->br->custom_locations) {
        rule->br->custom_locations = ngx_array_create(
            r->pool, 1, sizeof(ngx_http_custom_rule_location_t));
        if (!rule->br->custom_locations)
          return (NGX_CONF_ERROR);
      }
...
#define MZ_SPECIFIC_URL_X "$URL_X:"
      ...
      */
      if (rule->br->rx_mz == 1) {
        ...
        custom_rule->target_rx =
            ngx_pcalloc(r->pool, sizeof(ngx_regex_compile_t));
      ...
      custom_rule->hash =
          ngx_hash_key_lc(custom_rule->target.data, custom_rule->target.len);
...
void *dummy_whitelist(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule) {
  ...
  NX_LOG_DEBUG(_debug_whitelist, NGX_LOG_EMERG, r, 0, "%s %i", str.data,
               str.len);
  ...
  if (!wl_ar)
    return (NGX_CONF_ERROR);
  NX_LOG_DEBUG(_debug_whitelist, NGX_LOG_EMERG, r, 0,
               "XX- allocated %d elems for WL", ct);
  for (i = 0; i < str.len; i++) {
    if (i == 0 || str.data[i - 1] == ',') {
...
void *dummy_pass(ngx_conf_t *r, ngx_str_t *tmp, ngx_http_rule_t *rule) {
  ...
  str.data = tmp->data + strlen(PASS_T);
  str.len = tmp->len - strlen(PASS_T);
  NX_LOG_DEBUG(_debug_pass, NGX_LOG_EMERG, r, 0, "%s %i", str.data, str.len);

  for (ct = 1, i = 0; i < str.len; i++)
    if (str.data[i] == ',')
      ct++;
  pass_ar = ngx_array_create(r->pool, ct, sizeof(ngx_int_t));
  if (!pass_ar)
    return (NGX_CONF_ERROR);
  NX_LOG_DEBUG(_debug_pass, NGX_LOG_EMERG, r, 0,
               "XX- allocated %d elems for PASS", ct);
  for (i = 0; i < str.len; i++) {
    if (i == 0 || str.data[i - 1] == ',') {
      id = (ngx_int_t *)ngx_array_push(pass_ar);
      if (!id)
        return (NGX_CONF_ERROR);
      *id = (ngx_int_t)atoi((const char *)str.data + i);
    }
  }
  rule->passid_array = pass_ar;
  return (NGX_CONF_OK);
}
...
*/
void *ngx_http_dummy_cfg_parse_one_rule(ngx_conf_t *cf, ngx_str_t *value,
