...
class rules_extractor(object):
   ...
   def gen_basic_rules(self,url=None, srcip=None, dsthost=None,
     ...
     tmp_rules = []
     data = self.wrapper.getWhitelist()
     for row in data:
...
#      return self.base_rules, self.final_rules
   ...
   def generate_stats(self):
      stats = ""
      self.wrapper.execute("select count(distinct md5) as uniq_exception from exception")
      uniq_ex = self.wrapper.getResults()[0]['uniq_exception']
      self.wrapper.execute("select count(distinct peer_ip) as uniq_peer from peer where peer_ip is not NULL")
      uniq_peer = self.wrapper.getResults()[0]['uniq_peer']
      self.wrapper.execute("select count(distinct peer_ip) as uniq_peer_mon from http_monitor where peer_ip is not NULL")
      uniq_peer_mon = self.wrapper.getResults()[0]['uniq_peer_mon']
      self.wrapper.execute("select count(distinct md5) as uniq_exception_mon from http_monitor where md5 is not NULL")
      uniq_exception = self.wrapper.getResults()[0]['uniq_exception_mon']
...
class GraphView(Resource):
   ...
   def render_GET(self, request):
      ...
      self.ex.wrapper.execute('select p.peer_ip as ip, count(exception_id) as c from connections join peer as p on (src_peer_id = p.peer_id) group by p.peer_ip order by count(distinct exception_id) DESC limit 10;')
      ...
      self.ex.wrapper.execute('select distinct url, count(exception_id) as c from exception  group by url order by count(exception_id) DESC limit 10;')
   ...
   def build_js_array(self, id_beg = None, id_end = None):
      if id_beg is None or id_end is None:
         self.ex.wrapper.execute('select substr(date,1,10) as d, count(exception_id) as ex from connections group by substr(date,1,10)')
      else:
         self.ex.wrapper.execute('select substr(date,1,10) as d, count(co.exception_id) as ex from connections as co join match_zone as m on (co.match_id = m.match_id) where m.rule_id >= %s and m.rule_id <= %s group by substr(date,1,10);', (str(id_beg), str(id_end)))
